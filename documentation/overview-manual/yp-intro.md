...
> Yocto 项目的输出可以轻松地在不同的架构之间传输，而无需更换新的开发环境。此外，如果您使用 Yocto 项目创建了一个图像或应用程序，但发现自己无法支持它，Wind River，Mentor Graphics，Timesys 和 ENEA 等商业 Linux 供应商可以采用它并提供持续支持。这些供应商提供的产品是使用 Yocto 项目构建的。
...
> 项目工作流程可能会令人困惑：如果您习惯于传统的桌面和服务器软件开发，则 Yocto 项目工作流程 <overview-manual/development-environment：the yocto project development environment>{.interpreted-text role="ref"}可能会令人困惑。在桌面开发环境中，有一些机制可以轻松拉取和安装新的软件包，这些软件包通常是从可以通过互联网访问的服务器上预先编译的二进制文件。使用 Yocto 项目，您必须修改配置并重新构建以添加额外的软件包。
...
> 熟悉:yocto_home:[Yocto 项目兼容层\</software-overview/layers/\>]{.title-ref}或:oe_layerindex:[OpenEmbedded 层索引\<\>]{.title-ref}。后者包含更多的层，但它们的验证不那么普遍。
...
> 支持将技术、硬件组件和软件组件包含在图层中。“Yocto Project Compatible”（<dev-manual/layers:making sure your layer is compatible with yocto project>）{.interpreted-text role="ref"}认证提供了一定的最低标准，有助于形成强大的生态系统。“YP Compatible”被应用于适当的产品和软件组件，如 BSP，其他 OE 兼容层和相关的开源项目，使生产商可以使用 Yocto Project 徽章和品牌资产。
> :::
...
> 可扩展软件开发工具包 (eSDK)：eSDK 提供了一个跨开发工具链和库，专门针对特定图像的内容进行设计。eSDK 可以轻松地将新的应用程序和库添加到图像中，修改现有组件的源代码，在目标硬件上测试更改，并将其集成到 OpenEmbedded 构建系统的其余部分中。eSDK 为您提供了一个工具链体验，并补充了为 Yocto Project 环境定制的强大的 `devtool` 命令集。
...
> 烹饪报告系统：烹饪报告系统跟踪 Yocto 项目可用的食谱版本。该系统的主要目的是帮助您管理您维护的食谱，并提供动态的项目概览。烹饪报告系统是建立在:oe_layerindex:[开放式嵌入式层次索引 \<\>]{.title-ref}之上的，这是一个索引开放式嵌入式核心层的网站。
...
> BitBake 是 Yocto 项目的核心组件，被 OpenEmbedded 构建系统用来构建图像。虽然 BitBake 对构建系统至关重要，但它与 Yocto 项目是独立维护的。
...
> Opkg 旨在用于嵌入式 Linux 设备，并在此能力中使用：oe_home:[OpenEmbedded \<\>]{.title-ref} 和 [OpenWrt](https://openwrt.org/) 项目以及 Yocto 项目。
...
> 本地 Linux 主机：最佳构建主机的选择。使用 Linux 作为其本机操作系统的系统允许您直接使用 `BitBake`{.interpreted-text role="term"}工具开发软件。您可以从受支持的 Linux 发行版的常规 shell 中完成所有开发方面。
...
> Poky 是一个组合仓库，包括 BitBake、OpenEmbedded-Core（可在 `meta` 中找到）、`meta-poky`、`meta-yocto-bsp` 以及文档，它们能够很好地一起工作。您可以在 :yocto\_[git:%60Source](git:%60Source) Repositories \</poky/tree/\>\`中查看构成 Poky 仓库的项目。
...
> 开放式嵌入式核心（OE-Core）元数据，其中包括共享配置、全局变量定义、共享类、打包和配方。类定义构建逻辑的封装和继承。配方是要构建的软件和图像的逻辑单元。
...
> 关于 Poky 作为“默认配置”已经有很多讨论。默认配置提供了一个起始的图像足迹。您可以直接使用 Poky 来创建一个从可以访问 shell 的最小图像到使用基于 GNOME Mobile 和嵌入式(GMAE)的参考用户界面 Sato 的 Linux 标准基础(LSB)兼容图像的图像。
...
> 一个最强大的 Poky 属性是每个构建的方面都由元数据控制。您可以使用元数据通过添加元数据层 <overview-manual/yp-intro：yocto 项目层模型 >{.interpreted-text role="ref"}来增强这些基本图像类型。这些层可以提供例如为图像类型提供额外的软件堆栈，为其他硬件添加板支持包（BSP），甚至创建新的图像类型。
...
> 开放式嵌入式构建系统使用“工作流程”来完成图像和 SDK 生成。下图概述了该工作流程：
...
> 在 Yocto 项目的上下文中，此术语指的是 BitBake（即“烘焙食谱”）产生的配方输出的包装。包通常是从配方源代码编译生成的二进制文件。通过运行 BitBake 可以“烘焙”东西。
...
> 另一个值得注意的一点是，在 Yocto 项目的历史上，食谱被称为包-因此，存在一些似乎命名不当的 BitBake 变量（例如 `PR`{.interpreted-text role="term"}, `PV`{.interpreted-text role="term"} 和 `PE`{.interpreted-text role="term"}）。
...
> 食谱：最常见的元数据形式。食谱包含一系列设置和任务（即指令），用于构建用于构建二进制图像的包。食谱描述了您获取源代码的位置以及应用哪些补丁。食谱还描述了库或其他食谱的依赖关系以及配置和编译选项。相关的食谱被合并到一个层中。
