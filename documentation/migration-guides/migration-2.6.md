...
For names of recipes removed because of this repository change, see the `migration-guides/migration-2.6:removed recipes` section.

> 对于因此存储库更改而被移除的 recipes 名称，请参阅“迁移指南/迁移 2.6：移除 recipes”部分。

# `distutils` and `distutils3` Now Prevent Fetching Dependencies During the `do_configure` Task

Previously, it was possible for Python recipes that inherited the `distutils` and `distutils3` classes to fetch code during the `ref-tasks-configure`).

> 以前，继承了 `distutils` 和 `distutils3` 类的 Python recipes 可以在 `ref-tasks-configure` 任务期间获取代码，以满足 `setup.py` 中提到的依赖项，如果这些依赖项在 sysroot 中没有提供(即 `DEPENDS` 中缺少提供这些依赖项的 recipes)。
...
This change affects classes beyond just the two mentioned (i.e. `distutils` and `distutils3`). Any recipe that inherits `distutils*` classes are affected. For example, the `setuptools` and `ref-classes-setuptools3` recipes are affected since they inherit the `distutils*` classes.

> 这个改变不仅影响到前面提到的两个类(即 `distutils` 和 `distutils3`)。任何继承 `distutils*` 类的配方都受到影响。例如，`setuptools` 和 `ref-classes-setuptools3` 配方受到影响，因为它们继承了 `distutils*` 类。
> :::
...
> 从系统根目录中获取这些未提供的依赖项会影响重现构建的能力。现在明确禁止这种获取方式。因此，使用这些类的 Python 配方中的任何缺失依赖项现在会在 `ref-tasks-configure` 任务期间导致错误。
...
- Name variables (e.g. `IMAGE_NAME` allows easier and more direct changes.
...
- The `KERNEL_DTB_NAME` variables have been introduced to control kernel Device Tree Binary (DTB) artifact names instead of mangling `KERNEL_IMAGE_*` variables.

> 引入了 `KERNEL_DTB_NAME` 变量，以控制内核设备树二进制(DTB)工件名称，而不是混淆 `KERNEL_IMAGE_*` 变量。

- The `KERNEL_FIT_NAME` variables have been introduced to specify the name of flattened image tree (FIT) kernel images similar to other deployed artifacts.

> 引入了 `KERNEL_FIT_NAME` 这两个变量，以指定压缩映像树(FIT)内核映像的名称，与其他部署的工件类似。

- The `MODULE_TARBALL_NAME` variable values no longer include the \"module-\" prefix or \".tgz\" suffix. These parts are now hardcoded so that the values are consistent with other artifact naming variables.

> `MODULE_TARBALL_NAME` 变量的值不再包括“module-”前缀或“.tgz”后缀。这些部分现在是硬编码的，以便值与其他工件命名变量一致。

- Added the `INITRAMFS_LINK_NAME` variable so that the symlink can be controlled similarly to other artifact types.
- `INITRAMFS_NAME`\", which makes it consistent with other variables.

> INITRAMFS_NAME 现在使用"\$"，这使它与其他变量一致。
...
> 如果配置脚本报告有未知选项，现在会触发 QA 错误而不是警告。以前可以指定这种未知选项的 recipes 现在需要修复。
...
> - `forcevariable` 覆盖现在比 `libc` 覆盖优先级更高：`forcevariable` 覆盖已被记录为最高优先级覆盖。然而，由于 `OVERRIDES` 设置的长期怪癖，`libc` 覆盖(例如 `libc-glibc`，`libc-musl` 等)错误地具有更高的优先级。现在已经纠正了这个问题。
...
- The `build-$`, which is typically `build-linux`, override has been removed because building on a host operating system other than a recent version of Linux is neither supported nor recommended. Dropping the override avoids giving the impression that other host operating systems might be supported.

> 已移除 `build-$` 覆盖已被移除，因为在非最新版本的 Linux 主机操作系统上构建是不受支持也不建议的。去掉覆盖可以避免给其他主机操作系统带来可能被支持的错觉。
...
See the \"`bitbake-user-manual/bitbake-user-manual-metadata:removal (override style syntax)`\" section in the BitBake User Manual for a detailed example.
...
> 配置文件已经从 `systemd` recipes 移到 `system-conf` recipes 中。将此配置移到单独的 recipes 中可以避免 `systemd` recipes 变得特定于机器，以便在需要应用特定于机器的配置(例如 `qemu*` 机器)的情况下使用。
...
> 在本次发布之前，您可以将 `TEST_IMAGE` 变量设置为“1”以启用成功构建的镜像的自动测试。`TEST_IMAGE` 变量不再存在，已由 `TESTIMAGE_AUTO` 变量替换。

- Inheriting the `ref-classes-testimage` classes used for automatic testing.

> 继承 `ref-classes-testimage` 类。
...
> 你必须明确标记 post 安装以延迟到目标。如果你想明确将 postinstall 延迟到目标上的第一次启动而不是在根文件系统创建时间，请使用 `pkg_postinst_ontarget()` 或从 `pkg_postinst()` 调用 `postinst_intercept delay_to_first_boot`。任何 `pkg_postinst()` 脚本的失败(包括退出 1)都会在 `ref-tasks-rootfs` 任务中触发错误。

For more information on post-installation behavior, see the \"`dev-manual/new-recipe:post-installation scripts`\" section in the Yocto Project Development Tasks Manual.

> 要了解有关安装后行为的更多信息，请参阅 Yocto 项目开发任务手册中的“dev-manual/new-recipe：安装后脚本”部分。

# Python 3 Profile-Guided Optimization

The `python3` recipe now enables profile-guided optimization. Using this optimization requires a little extra build time in exchange for improved performance on the target at runtime. Additionally, the optimization is only enabled if the current `MACHINE`, which it is by default).

> 现在，`python3` recipes 支持面向概要的优化。使用此优化需要额外的构建时间，以换取目标在运行时的改进性能。此外，仅当当前的 `MACHINE` 中有“qemu-usermode”，默认情况下是这样)时，才会启用优化。

If you wish to disable Python profile-guided optimization regardless of the value of `MACHINE_FEATURES` for the `python3` recipe does not contain \"pgo\". You could accomplish the latter using the following at the configuration level:

> 如果您希望无论 `MACHINE_FEATURES` 中的 `python3` 配方不包含“pgo”。您可以使用以下配置级别来实现：
...
> `NOISO` 和 `NOHDD` 变量不再使用。您现在可以直接使用 `IMAGE_FSTYPES` 变量来控制生成 `*.iso` 和 `*.hddimg` 镜像类型。
...
- `$ variable directly. The remaining definitions from ` meta/conf/distro/include/as-needed.inc` have been moved to corresponding recipes.

> - 目前已经将 `$` 直接包含在 `TARGET_LDFLAGS` 变量中。来自 `meta/conf/distro/include/as-needed.inc` 的其余定义已经移动到相应的配方中。
...
> 现在，`dhcp` recipes 使用 `dhcpd6.service` 中的 `dhcpd6.conf` 配置文件来进行 IPv6 DHCP，而不是重复使用 `dhcpd.conf`，后者现在仅供 IPv4 使用。
