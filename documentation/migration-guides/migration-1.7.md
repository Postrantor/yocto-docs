...
The QEMU recipe now uses a number of `PACKAGECONFIG` for `qemu-native` and `nativesdk-qemu` instead of setting it. In other words, to enable graphical output for QEMU, you should now have these lines in `local.conf`:

> QEMU recipes 现在使用多个 PACKAGECONFIG 选项来启用各种可选功能。用于设置这些选项的默认值的方法意味着现有的 local.conf 文件将需要修改以附加到 qemu-native 和 nativesdk-qemu 的 PACKAGECONFIG 而不是设置它。换句话说，为了启用 QEMU 的图形输出，您现在应该在 local.conf 中有这些行：
...
The minimum `overview-manual/development-environment:git`\" section for more information.
...
The `module_autoload_* <module_autoload>`.

> 变量 `module_autoload_*<module_autoload>` 中。
...
- Additional QA checks `file-rdeps` and `build-deps` have been added in order to verify that file dependencies are satisfied (e.g. package contains a script requiring `/bin/bash`) and build-time dependencies are declared, respectively. For more information, please see the \"`/ref-manual/qa-checks`\" chapter.

> 除此之外，为了验证文件依赖性是否满足(例如，包含一个需要“/bin/bash”的脚本)，并且声明构建时间依赖关系，已添加了 `文件-rdeps` 和 `构建-deps` 的额外 QA 检查。有关更多信息，请参见“`/ref-manual/qa-checks`”章节。

- Package QA checks are now performed during a new `ref-tasks-package_qa` task as well.

> 现在在新的 `ref-tasks-package_qa` 任务。

- Files being overwritten during the `ref-tasks-populate_sysroot` task now trigger an error instead of a warning. Recipes should not be overwriting files written to the sysroot by other recipes. If you have these types of recipes, you need to alter them so that they do not overwrite these files.

> 在 `ref-tasks-populate_sysroot` 任务期间被覆盖的文件现在会引发错误而不是警告。recipes 不应该覆盖由其他 recipes 写入的 sysroot 文件。如果您有这些类型的 recipes，您需要改变它们，以便它们不会覆盖这些文件。
...
> - `ubootchart`：此 recipes 已过时。已添加 `bootchart2` recipes 以功能上取代它。
...
- `eglibc` has been removed in favor of `glibc`. See the \"`migration-1.7-glibc-replaces-eglibc`\" section for more information.

> `- ` Eglibc `已被` glibc ` 替代。有关更多信息，请参阅“` migration-1.7-glibc-replaces-eglibc`”部分。
