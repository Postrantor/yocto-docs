...
- `cve-check-tool`: Functionally replaced by the `cve-update-db` recipe and `ref-classes-cve-check` class.

> `cve-check-tool`：已被 `cve-update-db` 配方和 `ref-classes-cve-check` 类所取代。
...
- The `Epiphany <GNOME_Web>` browser has been dropped from `packagegroup-self-hosted` as it has not been needed inside `build-appliance-image` for quite some time and was causing resource problems.
...
`cve-check-tool` has been functionally replaced by a new `cve-update-db` recipe and functionality built into the `ref-classes-cve-check` class. The result uses NVD JSON data feeds rather than the deprecated XML feeds that `cve-check-tool` was using, supports CVSSv3 scoring, and makes other improvements.
...
Additionally, the `CVE_CHECK_CVE_WHITELIST` variable has been replaced by `CVE_CHECK_WHITELIST` (replaced by `CVE_CHECK_IGNORE` in version 3.5).
...
- The arguments passed to functions used with `BB_HASHCHECK_FUNCTION` have changed. If you are using your own custom hash check function, see :yocto_[git:%60/poky/commit/?id=40a5e193c4ba45c928fccd899415ea56b5417725](git:%60/poky/commit/?id=40a5e193c4ba45c928fccd899415ea56b5417725)\` for details.
...
- `SRC_URI` is now checked for usage of two problematic items:
  ...
  - \"\$ is used. You must fix the issue regardless of whether multiconfig or anything else that would cause prefixing/suffixing to happen.

> "\$。无论是多种配置还是其他导致前缀/后缀发生的情况，都必须解决问题。
...
- Setting `DEPENDS_$ instead.

> 设置 `DEPENDS_$` 任何地方(例如通常在 recipes 中)现在会触发错误。触发错误是因为 `DEPENDS` 不像 RDEPENDS 那样是特定于软件包的变量。你应该设置 `DEPENDS` 而不是它。
...
> 因为 `gnome` 类现在几乎没有什么作用，所以已经被移除。您应该更新先前继承此类的 recipes，以执行以下操作：
...
- `TARGET_CFLAGS` are no longer exported to the external environment. This change did not require any changes to core recipes, which is a good indicator that no changes will be required. However, if for some reason the software being built by one of your recipes is expecting these variables to be set, then building the recipe will fail. In such cases, you must either export the variable or variables in the recipe or change the scripts so that exporting is not necessary.

> `TARGET_CFLAGS` 不再被导出到外部环境。这个变化并不需要对核心配方做任何改变，这是一个很好的指标，表明不需要做任何改变。但是，如果由于某些原因，您的配方构建的软件期望这些变量被设置，那么构建配方将会失败。在这种情况下，您必须在配方中导出变量或变量，或者更改脚本以便不需要导出。

- You must change the host distro identifier used in `NATIVELSBSTRING`.

> 你必须把 `NATIVELSBSTRING` 时，才需要这样做。
...
> UEFI 镜像变量设置已从多个位置移动到中央的“conf/image-uefi.conf”。此更改不应影响任何现有配置，因为核心元数据中的“meta/conf/image-uefi.conf”设置了默认值，可以以与以前相同的方式进行覆盖。
