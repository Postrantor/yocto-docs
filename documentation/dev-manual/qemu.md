...
2. *Setting Up the Environment:* How you set up the QEMU environment depends on how you installed QEMU:
   ...
   - If you cloned the `poky` repository or you downloaded and unpacked a Yocto Project release tarball, you can source the build environment script (i.e. `structure-core-script`):

> 如果您克隆了 `poky` 存储库或下载并解压缩了 Yocto Project 发行 tarball，您可以源自构建环境脚本(即 `structure-core-script`)：
...
- If you have not built an image, you can go to the :yocto_dl:[machines/qemu \</releases/yocto/yocto-&DISTRO;/machines/qemu/\>] area and download a pre-built image that matches your architecture and can be run on QEMU.

> 如果您还没有构建镜像，您可以转到:yocto_dl:[机器/qemu \</releases/yocto/yocto-&DISTRO;/机器/qemu/\>] 区域并下载与您的架构匹配并可在 QEMU 上运行的预构建镜像。

See the \"`sdk-manual/appendix-obtain:extracting the root filesystem`\" section in the Yocto Project Application Development and the Extensible Software Development Kit (eSDK) manual for information on how to extract a root filesystem.
...
> 这个例子使用 MACHINE 设置为“qemux86-64”启动 QEMU。假设有一个标准的“构建目录”，`runqemu` 会自动找到 `bzImage-qemux86-64.bin` 镜像文件和 `core-image-minimal-qemux86-64-20200218002850.rootfs.ext4`(假设当前构建创建了一个 `core-image-minimal` 镜像)：
...
- This example specifies to boot an `Initramfs` for more information):

> 这个例子指定要启动一个 Initramfs 镜像，并在 QEMU 中启用音频。对于此情况，runqemu 将内部变量 FSTYPE 设置为 cpio.gz。此外，为了启用音频，必须安装适当的驱动程序(有关更多信息，请参见 dev-manual/qemu：runqemu 命令行选项中的 audio 选项)：
...
- This example does not provide enough information for QEMU to launch. While the command does provide a root filesystem type, it must also minimally provide a [MACHINE] option:

> 这个示例没有提供足够的信息让 QEMU 启动。虽然该命令提供了根文件系统类型，但它必须至少提供[MACHINE]选项：
...
> 2. *启动 QEMU：*一旦您提取了文件系统，就可以使用文件系统的额外位置正常运行 `runqemu`。然后，您还可以对 `./test-nfs` 中的文件进行更改，并实时在镜像中看到这些更改。这里是使用 `qemux86` 映像的示例：
...
> 使用 QEMU 来模拟你的硬件可能会根据目标和主机架构混合而导致速度问题。例如，在 Intel 基于 32 位(x86)的主机机器上使用模拟器中的 `qemux86` 镜像会很快，因为目标和主机架构匹配。另一方面，在同一 Intel 基础的主机上使用 `qemuarm` 镜像可能会更慢。但是，你仍然可以实现 ARM 特定问题的忠实模拟。
...
> 为了加快速度，QEMU 镜像支持使用 `distcc` 从模拟系统之外调用跨编译器。如果您使用 `runqemu` 启动 QEMU，并且主机系统上存在 `distccd` 应用程序，则可以通过调用 `distcc` 从构建系统中自动使用任何 BitBake 跨编译工具链。您可以通过定义跨编译器变量(例如 `export CC="distcc"`)来实现此目的。或者，如果使用适当的 SDK 映像或存在适当的独立工具链，则还会自动使用该工具链。
...
> - \`VM\`：虚拟机映像，必须是 `.wic.vmdk` 文件。当您想启动 `.wic.vmdk` 映像时，使用此选项。您提供的镜像文件名必须包含以下字符串之一：“qemux86-64”，“qemux86”，“qemuarm”，“qemumips64”，“qemumips”，“qemuppc”或“qemush4”。
...
> 默认情况下，使用 `slirp` 将客户机的 22 和 23 TCP 端口转发到主机的 2222 和 2323 端口(或下一个空闲端口)。可以通过设置 `QB_SLIRP_OPT` 环境变量或 `qemuboot.conf` 中的 `Build Directory` `deploy/image` 目录来配置特定的转发规则。示例：
