...
> 利用 OpenEmbedded 构建系统为特定硬件目标创建镜像并不意味着你可以直接在设备上启动该镜像。物理设备以不同的方式接受和启动镜像，这取决于设备的具体情况。通常，有关硬件的信息可以告诉您设备所需的镜像格式。如果您的设备需要 SD 卡、闪存或硬盘驱动器上的多个分区，您可以使用 OpenEmbedded Image Creator(Wic)创建正确分区的镜像。
...
> `wic` 命令可以从现有的 OpenEmbedded 构建工件中生成分区镜像。镜像生成由 OpenEmbedded 启动文件(`.wks`)中包含的分区命令驱动，该文件可以直接在命令行中指定，也可以作为“dev-manual/wic：使用现有启动文件生成镜像”部分中使用 `wic list images` 命令显示的选择性启动文件之一。当您将该命令应用于给定的构建工件时，结果将是一个或多个可以直接写入介质并在特定系统上使用的镜像。
...
For a kickstart file reference, see the \"``ref-manual/kickstart:openembedded kickstart (\`\`.wks\`\`) reference``\" Chapter in the Yocto Project Reference Manual.

> 对于启动文件参考，请参见 Yocto 项目参考手册中的“ref-manual/kickstart：开放嵌入式启动(``.wks``)参考”章节。
> :::

The `wic` command and the infrastructure it is based on is by definition incomplete. The purpose of the command is to allow the generation of customized images, and as such, was designed to be completely extensible through a plugin interface. See the \"`dev-manual/wic:using the wic plugin interface`\" section for information on these plugins.

> `wic` 命令及其基础架构本质上是不完整的。该命令的目的是允许生成定制的镜像，因此设计为通过插件接口完全可扩展。有关这些插件的信息，请参阅“dev-manual/wic：使用 wic 插件接口”部分。
...
> 这个名字“Wic”源自开放式嵌入式镜像创建者(oeic)。“oeic”中的“oe”双元音被提升为字母“w”，因为“oeic”既难以记忆又难以发音。
...
> Wic 是基于 Meego 镜像创建器(`mic`)框架松散地构建的。Wic 的实现已经进行了大量修改，以直接使用 OpenEmbedded 构建工件，而不是已经包含在 OpenEmbedded 工件中的包安装和配置。

- Wic is a completely independent standalone utility that initially provides easier-to-use and more flexible replacements for an existing functionality in OE-Core\'s `ref-classes-image-live` class. The difference between Wic and those examples is that with Wic the functionality of those scripts is implemented by a general-purpose partitioning language, which is based on Redhat kickstart syntax.

> Wic 是一个完全独立的 standalone 实用程序，最初为 OE-Core 的 `ref-classes-image-live` 类提供更易于使用和更灵活的替代方案。Wic 与这些示例的区别在于，使用 Wic，这些脚本的功能由基于 Redhat kickstart 语法的通用分区语言实现。

# Requirements

In order to use the Wic utility with the OpenEmbedded Build system, your system needs to meet the following requirements:

- The Linux distribution on your development host must support the Yocto Project. See the \"`system-requirements-supported-distros`\" section in the Yocto Project Reference Manual for the list of distributions that support the Yocto Project.
...
- You must have sourced the build environment setup script (i.e. `structure-core-script`.

> 你必须源自构建环境设置脚本(即在“构建目录”中找到的 `structure-core-script`)。
...
- Include the name of the `wic kickstart file <openembedded-kickstart-wks-reference>` variable instead.

> 包括 `WIC Kickstart文件<openembedded-kickstart-wks-reference>` 变量指定所有可能的名称。
...
> 对于 Wic，还有一个额外的帮助级别。您可以通过“list”命令获取有关单个镜像的帮助。您可以使用“list”命令如下返回可用的 Wic 镜像：
...
> 一旦你知道可用的 Wic 镜像的列表，你可以使用 `help` 命令来获取关于特定镜像的帮助。例如，下面的命令返回关于“beaglebone-yocto”镜像的帮助：
...
Running Wic in raw mode allows you to specify all the partitions through the `wic` command line. The primary use for raw mode is if you have built your kernel outside of the Yocto Project `Build Directory` (e.g. `tmp/deploy/images/` machine).

> 运行 Wic 的原始模式允许您通过 `wic` 命令行指定所有分区。原始模式的主要用途是如果您在 Yocto Project `Build Directory`(例如 `tmp/deploy/images/` 机器)中查找。
...
Running Wic in cooked mode leverages off artifacts in the `Build Directory` (e.g. `tmp/deploy/images/` machine) for artifacts.

> 运行 Wic 的烹饪模式利用 `构建目录` 中的工件。换句话说，您不必将内核或根文件系统位置作为命令的一部分指定。您所需要提供的是 kickstart 文件和要使用的镜像的名称，使用“-e”选项。Wic 在 `构建目录`(例如 `tmp/deploy/images/` 机器)中查找工件。
...
If you do not want to create your own kickstart file, you can use an existing file provided by the Wic installation. As shipped, kickstart files can be found in the `overview-manual/development-environment:yocto project source repositories` in the following two locations:
...
> 源插件提供了一种机制，可以在 Wic 镜像生成过程中自定义分区内容。您可以使用源插件将使用 kickstart 文件(即 `*.wks`)中的 `--source` 命令指定的值映射到用于填充给定分区的插件实现。
...
Source plugins are subclasses defined in plugin files. As shipped, the Yocto Project provides several plugin files. You can see the source plugin files that ship with the Yocto Project :yocto_[git:%60here](git:%60here) \</poky/tree/scripts/lib/wic/plugins/source\>\`. Each of these plugin files contains source plugins that are designed to populate a specific Wic image partition.

> 源插件是在插件文件中定义的子类。按照出货，Yocto 项目提供了几个插件文件。您可以看到 Yocto 项目附带的源插件文件：yocto_[git:`here`](git:%60here%60) \</poky/tree/scripts/lib/wic/plugins/source\>\`。每个这些插件文件都包含旨在填充特定 Wic 镜像分区的源插件。
...
This section provides several examples that show how to use the Wic utility. All the examples assume the list of requirements in the \"`dev-manual/wic:requirements`\" section have been met. The examples assume the previously generated image is `core-image-minimal`.

> 这一节提供几个示例，展示如何使用 Wic 工具。所有示例假设已经满足了“dev-manual/wic：requirements”部分中的要求列表。示例假设之前生成的镜像是 `core-image-minimal`。
...
> 前面的例子展示了在熟练模式下创建镜像的最简单方法，并提供了 kickstart 文件和“-e”选项来指向现有的构建工件。您的 `local.conf` 文件需要将 `MACHINE` 变量设置为您正在使用的机器，在本例中为“qemux86”。
...
For more information on how to use the `bmaptool` to flash a device with an image, see the \"``dev-manual/bmaptool:flashing images using \`\`bmaptool\`\` ``\" section.

> 如何使用 `bmaptool` 烧录设备镜像的更多信息，请参见“dev-manual/bmaptool：使用 `bmaptool` 烧录镜像”部分。
> :::
...
> 在这个例子中，现有的 `directdisk-gpt` 文件已经完成了大部分所需的工作。但是，对于这个例子中的硬件，镜像将需要从 `sdb` 而不是 `sda` 引导，而 `directdisk-gpt` 引导文件使用的是 `sda`。
...
> 继续以此例，您现在可以直接将镜像 `dd` 到 USB 存储设备或您为其制作镜像的任何媒体上，并启动生成的媒体：
...
> Wic 镜像处理可以缩短镜像开发过程中的周转时间。例如，您可以使用 Wic 删除 Wic 镜像的内核分区，然后插入新构建的内核。这样，您就不必每次修改内核时都重新构建整个镜像，从而节省了时间。
...
> 使用 `wic cp` 命令将更新后的内核添加到 Wic 镜像中。根据您如何构建内核，它可能会放在不同的地方。如果您使用 `devtool` 和 SDK 构建内核，它将位于可扩展 SDK 的 `tmp/work` 目录中。如果您使用 `make` 构建内核，则内核将位于 `workspace/sources` 区域中。
...
Once the new kernel is added back into the image, you can use the `dd` command or ``bmaptool <dev-manual/bmaptool:flashing images using \`\`bmaptool\`\`>`` to flash your wic image onto an SD card or USB stick and test your target.
