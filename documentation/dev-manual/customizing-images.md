...
> 可能最简单的自定义镜像的方法是通过 `local.conf` 配置文件添加一个包。由于它仅限于本地使用，因此此方法通常仅允许添加软件包，而不如创建自己的自定义镜像那么灵活。当您使用此方法添加软件包时，您需要意识到这些变量更改对每次构建都有效，并因此影响所有镜像，这可能不是您所需要的。
...
> 使用本地配置文件向镜像添加包，请使用带有 `:append` 操作符的 `IMAGE_INSTALL` 变量：
...
> 此外，如果要避免排序问题，必须使用 `:append` 而不是 `+=` 操作符。这是因为使用它可以无条件地附加到变量中，从而避免由于使用 `?=` 等操作符在镜像配方和 `.bbclass` 文件中设置变量而导致的排序问题。使用 `:append` 可以确保操作生效。
...
> IMAGE_INSTALL:append 影响所有镜像的最简单用法如上所示。可以扩展语法，使变量仅适用于特定镜像。这里有一个例子：
...
> 你可以通过类似的方法使用 `CORE_IMAGE_EXTRA_INSTALL` 变量添加软件包。如果你使用这个变量，只有 `core-image-*` 镜像会受到影响。
...
> 另一种自定义镜像的方法是通过使用 `IMAGE_FEATURES` 和 `EXTRA_IMAGE_FEATURES` 变量来启用或禁用高级镜像功能。虽然这两个变量的功能几乎相同，但最佳实践是从配方中使用 `IMAGE_FEATURES`，从 `Build Directory` 中的 `local.conf` 文件中使用 `EXTRA_IMAGE_FEATURES`。

To understand how these features work, the best reference is `meta/classes-recipe/image.bbclass <ref-classes-image>` of which most map to package groups while some, such as `debug-tweaks` and `read-only-rootfs`, resolve as general configuration settings.

> 要了解这些功能如何工作，最佳参考是 `meta/classes-recipe/image.bbclass <ref-classes-image>`，其中大多数映射到软件包组，而一些，例如 `debug-tweaks` 和 `read-only-rootfs`，解析为一般配置设置。

In summary, the file looks at the contents of the `IMAGE_FEATURES` variable. Effectively, you are enabling extra features by extending the class or creating a custom class for use with specialized image `.bb` files.

> 总之，该文件研究 `IMAGE_FEATURES` 变量的内容，然后根据该信息映射或配置特征。根据此信息，构建系统会自动将适当的软件包或配置添加到 `IMAGE_INSTALL` 变量中。有效地，您可以通过为专用镜像 `.bb` 文件扩展类或创建自定义类来启用额外的功能。

Use the `EXTRA_IMAGE_FEATURES` within `meta/conf/bitbake.conf`.

> 使用本地配置文件中的 `EXTRA_IMAGE_FEATURES` 变量。使用一个单独的区域来启用此变量的功能可以帮助您避免覆盖镜像配方中使用 `IMAGE_FEATURES` 启用的功能。`EXTRA_IMAGE_FEATURES` 的值将添加到 `meta / conf / bitbake.conf` 中的 `IMAGE_FEATURES` 中。
...
> 为了说明你如何使用这些变量来修改你的镜像，考虑一个选择 SSH 服务器的例子。Yocto 项目附带了两个可以用于镜像的 SSH 服务器：Dropbear 和 OpenSSH。Dropbear 是一个适合资源受限环境的最小 SSH 服务器，而 OpenSSH 是一个众所周知的标准 SSH 服务器实现。默认情况下，`core-image-sato` 镜像配置为使用 Dropbear。`core-image-full-cmdline` 和 `core-image-lsb` 镜像都包含 OpenSSH。`core-image-minimal` 镜像不包含 SSH 服务器。

You can customize your image and change these defaults. Edit the `IMAGE_FEATURES` in your `local.conf` file so that it configures the image you are working with to include `ssh-server-dropbear` or `ssh-server-openssh`.
...
See the \"`ref-manual/features:image features`\" section in the Yocto Project Reference Manual for a complete list of image features that ship with the Yocto Project.

> 请参阅 Yocto Project 参考手册中的“ref-manual/features:image features”部分，了解 Yocto Project 所提供的完整镜像功能列表。
> :::
...
> 您也可以通过创建自定义配方来自定义镜像，该配方定义额外的软件作为镜像的一部分。以下示例显示了您需要的两行的表单：
...
> 另一种创建自定义镜像的方法是基于现有镜像。例如，如果要创建基于 `core-image-sato` 的镜像，但要将额外的软件包 `strace` 添加到镜像中，请将 `meta/recipes-sato/images/core-image-sato.bb` 复制到新的 `.bb` 文件，并在复制的末尾添加以下行：
...
> 对于复杂的自定义镜像，最佳的定制镜像方法是创建一个用于构建镜像或镜像的自定义软件包组配方。一个好的软件包组配方示例是 `meta/recipes-core/packagegroups/packagegroup-base.bb`。

If you examine that recipe, you see that the `PACKAGES` statement.

> 如果你检查该 recipes，你会发现 `PACKAGES` 变量列出了要生成的包组包。`inherit packagegroup` 语句设置合适的默认值，并且自动为 `PACKAGES` 语句中指定的每个包添加 `-dev`、`-dbg` 和 `-ptest` 补充包。
...
> `inherit packagegroup` 行应该位于 recipes 的顶部，肯定在 `PACKAGES` 语句之前。
> :::

For each package you specify in `PACKAGES` entries to provide a list of packages the parent task package should contain. You can see examples of these further down in the `packagegroup-base.bb` recipe.

> 对于您在 `PACKAGES` 条目提供父任务包应包含的包列表。您可以在 `packagegroup-base.bb` 配方中看到这些示例。
...
> 这是一个简短的虚构示例，用于显示在 `packagegroup-custom.bb` 中定义的假设的 packagegroup 的基本部分，其中变量 `PN` 是引用完整的 packagegroup 名称 `packagegroup-custom` 的标准方式：
...
> 在前面的例子中，使用它们的依赖关系及其推荐的软件包依赖关系创建了两个软件包组：`packagegroup-custom-apps` 和 `packagegroup-custom-tools`。要使用这些软件包组构建映像，您需要将 `packagegroup-custom-apps` 和/或 `packagegroup-custom-tools` 添加到 `IMAGE_INSTALL`。有关其他形式的映像依赖关系，请参阅本节的其他部分。
...
> 你可以通过修改"hostname"变量在 base-files recipes 中的值来自定义这个名称。使用以下内容在附加文件中：
...
> 更改变量“主机名”的默认值在某些情况下可能很有用。例如，假设您需要对镜像进行大量测试，并希望能够从具有典型默认主机名的现有镜像中轻松识别正在测试的镜像。在这种情况下，您可以将默认主机名更改为“testme”，这样所有镜像都将使用名称“testme”。一旦测试完成，您不再需要重新构建镜像进行测试，您可以轻松重置默认主机名。
