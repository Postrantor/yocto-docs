...
> 按照 Yocto 项目概览和概念手册中“概述-手册/开发环境：许可”部分提到的，开源项目对公众开放，因此它们有不同的许可结构。本节描述了 OpenEmbedded 构建系统如何跟踪许可文本的变化，以及如何在项目的生命周期中维护开源许可合规性。该节还描述了如何启用商业许可的食谱，默认情况下这些食谱是禁用的。
...
> 未来，上游项目的许可证可能会发生变化。为了防止这些变化不被注意到，`LIC_FILES_CHKSUM`{.interpreted-text role="term"}变量跟踪许可证文本的变化。校验和在配置步骤结束时进行验证，如果校验和不匹配，构建将失败。
...
> `LIC_FILES_CHKSUM`{.interpreted-text role="term"}变量包含食谱源代码中许可文本的校验和。以下是如何指定 `LIC_FILES_CHKSUM`{.interpreted-text role="term"}的示例：
...
> 系统在搜索 `LIC_FILES_CHKSUM`{.interpreted-text role="term"}中列出的文件时，默认使用 `S`{.interpreted-text role="term"}变量。前面的例子使用了默认目录。
...
> 第一行将文件定位在 `${S}/src/ls.c`，并将第五行到第十六行提取出来作为许可文本。第二行指的是 `WORKDIR`{.interpreted-text role="term"}中的文件。
...
> 同样的，为了使用包含 `LICENSE_FLAGS = "license_${PN}_${PV}"` 的配方所构建的包，假设实际的配方名称是 `emgd_1.10.bb`，以下字符串将启用该软件包以及原始的 `gst-plugins-ugly` 软件包：
...
> 许可标志匹配允许您控制 OpenEmbedded 构建系统在构建中包含哪些配方。从本质上讲，构建系统会尝试将配方中找到的 `LICENSE_FLAGS`{.interpreted-text role="term"}字符串与 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}字符串进行匹配。如果匹配成功，构建系统会将配方包含在构建中，如果未能找到匹配项，构建系统会排除该配方。
...
> 在用特定配方定义的标志被测试到 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}的条目之前，展开的字符串 `_${PN}` 会被附加到标志上。这个展开使每个 `LICENSE_FLAGS`{.interpreted-text role="term"}值特定于配方。展开后，字符串然后会被匹配到条目。因此，例如，在配方“foo”中指定 `LICENSE_FLAGS = "commercial"` 会导致字符串 `"commercial_foo"`。为了创建一个匹配，该字符串必须出现在 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}的条目中。
...
> 恰当地使用 `LICENSE_FLAGS`{.interpreted-text role="term"}字符串和 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}变量的内容可以让您根据许可证有很大的灵活性来包含或排除食谱。 例如，您可以通过在 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}中使用许可证标志字符串子集来扩大匹配能力。
...
> 例如，在 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}变量中只指定字符串“commercial”，就可以匹配以“commercial”开头的任何展开的 `LICENSE_FLAGS`{.interpreted-text role="term"}定义，比如“commercial_foo”和“commercial_bar”，这些字符串是假设这些食谱只指定以下内容时，构建系统自动生成的：
...
> 因此，您可以选择详尽地列出列表中的每个许可证标志，并仅允许特定配方进入图像，或者您可以使用引起更广泛匹配的字符子集，以允许一系列配方进入图像。
...
> 这个方案即使 `LICENSE_FLAGS`{.interpreted-text role="term"}字符串已经附加有 `_${PN}` 也能正常工作。例如，构建系统将许可标志“commercial_1.2_foo”转换为“commercial_1.2_foo_foo”，并且可以匹配 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}变量中的一般“commercial”和特定的“commercial_1.2_foo”字符串，如预期的那样。
...
> 你可以在食谱中指定一个版本字符串，比如在“foo”食谱中指定“commercial_foo_1.2”。构建系统会将此字符串扩展为“commercial_foo_1.2_foo”。将此许可标志与具有字符串“commercial”的 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}变量结合起来，即可与任何以“commercial”开头的标志匹配。
...
> 你可以非常具体地使用 `LICENSE_FLAGS_ACCEPTED` 列表中的包和版本部分（例如“commercial_foo_1.2”）来特定匹配一个版本化的食谱。
...
> 当然，您也可以使用更常见的“商业”字符串在 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}变量中创建一个匹配列表，但这也会启用所有其他包含“商业”的 `LICENSE_FLAGS`{.interpreted-text role="term"}，您可能会或可能不会想要：
...
> 指定音频和视频插件作为 `COMMERCIAL_AUDIO_PLUGINS`{.interpreted-text role="term"}和 `COMMERCIAL_VIDEO_PLUGINS`{.interpreted-text role="term"}语句（以及 `LICENSE_FLAGS_ACCEPTED`{.interpreted-text role="term"}）的一部分，将插件或组件包含到构建的图像中，从而添加对媒体格式或组件的支持。
...
> 随着不同组织遵守开源许可协议的方式不同，本节旨在描述一种实现合规的方法，而不是暗示只有一种方法可以满足您的合规义务。本节的其余部分描述了支持满足前述三个要求的方法。在释放图像、源代码和构建系统之前，您应该审核所有工件以确保完整性。
...
> 面向审计提供帮助，Yocto 项目在图像创建时会生成一个许可证清单，位于 `${DEPLOY_DIR}/licenses/`[image_name]{.title-ref}`-`[datestamp]{.title-ref}。
> :::
...
> 您在生成最终图像之前应该开始执行合规活动。首先要考虑的是大多数合规组织最重要的要求——提供源代码。Yocto Project 有几种满足这一要求的方法。
...
> 一个最简单的满足这个要求的方法是提供整个由构建使用的 `DL_DIR`。然而，这种方法有一些问题。最明显的是目录的大小，因为它包括构建中使用的所有源，而不仅仅是发布图像中使用的源。它将包括工具链源和其他工件，这些工件通常不会发布。然而，对于大多数公司而言，更严重的问题是意外发布专有软件。Yocto 项目提供了一个 `ref-classes-archiver` 类来帮助避免一些这些问题。
...
> 在使用 `DL_DIR`{.interpreted-text role="term"}或 `ref-classes-archiver`{.interpreted-text role="ref"}类之前，你需要决定如何提供源。`ref-classes-archiver`{.interpreted-text role="ref"}类可以生成 tarballs 和 SRPMs，并且可以根据不同级别的合规性创建它们。
...
> 在创建图像的过程中，根据每个配方的许可证，将部署到图像的所有配方的源代码放置在 `DEPLOY_DIR/sources` 的子目录中。发布整个目录可以帮助您遵守提供未修改源代码的要求。重要的是要注意，该目录的大小可能会变得很大。
...
> 将三个变量设置为“1”的结果是，图像具有两个相同的许可证文件副本。一个副本位于/usr/share/common-licenses，另一个副本位于/usr/share/license。
...
> 到目前为止，我们已经处理了生成图像所需的一切。接下来的两个要求是在最终发布时处理的。
...
> 如果部署团队有一个 `overview-manual/concepts:bsp layer`{.interpreted-text role="ref"}层和一个发行版层，并且这些层被用来修补、编译、打包或修改（以任何方式）您发布的图像中包含的任何开源软件，您可能需要根据 GPLv2 第 3 节或 GPLv3 第 1 节发布这些层。一种做法是使用您构建时使用的 Yocto 项目和层的干净检出版本。这里有一个例子：
...
> 一个开发组织可能想要考虑用户便利性的一件事是修改 `meta-poky/conf/templates/default/bblayers.conf.sample`，以确保当最终用户使用发布的构建系统构建图像时，开发组织的层会自动包含在 `bblayers.conf` 文件中。
...
> 创建和提供元数据层的存档（食谱、配置文件等）可以帮助您满足要求，包括用于控制编译的脚本以及对原始源代码的任何修改。
...
> 当通过 `RDEPENDS` 或 `RRECOMMENDS` 机制将包 A 添加到图像中，以及使用 `IMAGE_INSTALL` 明确地将其包含在图像配方中，并且依赖于静态链接库配方 B（`DEPENDS += "B"`）时，包 B 将不会出现在生成的许可证清单中，也不会出现在生成的源 tarballs 中。这是因为 `ref-classes-license` 和 `ref-classes-archiver` 类假定只有通过 `RDEPENDS` 或 `RRECOMMENDS` 添加到图像中的包才会出现。
