...
> 当您关心图像安全时，您可以考虑使用一些步骤、工具和变量来帮助您实现特定设备所需的安全目标。在使图像更安全时，并不是所有情况都完全相同。因此，本节提供了一些指导和建议，以供您在想要使图像更安全时参考。
...
> 有一些常规考虑可以帮助您创建更安全的图像。您应该考虑以下建议，以使您的设备更安全：
...
> 确保“debug-tweaks”不是您选择的 `IMAGE_FEATURES`{.interpreted-text role="term"}之一。在创建新项目时，默认情况下会为您提供一个初始的 `local.conf` 文件，使用 `EXTRA_IMAGE_FEATURES`{.interpreted-text role="term"}变量来启用此功能，行如下：
...
> 若要設定密碼，請使用「ref-classes-extrausers」{.interpreted-text role="ref"}類別，這是推薦的方法。 要瞭解如何設定 root 和使用者密碼的範例，請參閱「ref-classes-extrausers」{.interpreted-text role="ref"}部分。
...
> 项目 Yocto 提供了工具，可以使您的图像更安全。您可以在 Yocto 项目源存储库的 meta-security 层中找到这些工具。
