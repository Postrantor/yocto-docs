...
> 食谱（`.bb` 文件）是 Yocto 项目环境中的基本组件。由 OpenEmbedded 构建系统构建的每个软件组件都需要一个食谱来定义该组件。本节介绍如何创建、编写和测试一个新的食谱。
...
> 要了解有用于食谱的变量以及有关食谱命名问题的信息，请参阅 Yocto Project 参考手册中的“ref-manual/varlocality:recipes”部分。
> :::
...
> 在从头开始编写食谱之前，通常有用的是发现是否有其他人已经写了一个满足（或接近满足）您的需求的食谱。Yocto 项目和 OpenEmbedded 社区维护了许多可能是您正在做的候选食谱。您可以在 oe_layerindex：[OpenEmbedded Layer Index \<\>]{.title-ref}中找到一个很好的中央索引。
...
> 找到并修改接近你想要做的食谱：当你熟悉当前食谱空间时，这种方法就可行。对于那些初次接触 Yocto 项目或编写食谱的人来说，这种方法就不太好用了。
...
> 一些与此方法相关的风险包括使用一个与你试图通过你的食谱实现的目标完全无关的食谱，无法识别可能需要从头开始添加的食谱部分等等。所有这些风险源于对现有食谱空间的不熟悉。
...
> 如果您出于某种原因不想使用 `recipetool`，并且找不到满足您需求的现有食谱，您可以使用以下结构提供新食谱的基本领域：
...
> 创建一个新的食谱通常是一个迭代过程，需要使用 BitBake 多次处理食谱，以逐步发现和添加信息到食谱文件中。
...
> 在构建期间，OpenEmbedded 构建系统为每个配方（`${WORKDIR}`）创建一个临时工作目录，其中保存提取的源文件、日志文件、中间编译和打包文件等等。
...
> 假设一个名为 `poky` 的源目录顶级文件夹，默认的 `构建目录`{.interpreted-text role="term"}为 `poky/build`，以及 `qemux86-poky-linux` 机器目标系统。此外，假设您的配方名为 `foo_1.3.0.bb`。在这种情况下，构建系统用于构建软件包的工作目录如下：
...
> 你可以在食谱的 `temp` 目录中找到每个任务的日志文件（例如 `poky/build/tmp/work/qemux86-poky-linux/foo/1.3.0-r0/temp`）。日志文件的命名格式为 `log.taskname`（例如 `log.do_configure`、`log.do_fetch` 和 `log.do_compile`）。
> :::
...
> 首先，你的食谱必须指定如何获取源文件。获取主要通过 `SRC_URI` 变量控制。你的食谱必须有一个 `SRC_URI` 变量，指向源位置。有关源位置的图形表示，请参见 Yocto 项目概述和概念手册中的“概览-手册/概念：源”部分。
...
> 任务 `ref-tasks-fetch`{.interpreted-text role="ref"} 使用 `SRC_URI`{.interpreted-text role="term"} 变量值的每个条目的前缀来确定要使用哪个 `fetcher <bitbake-user-manual/bitbake-user-manual-fetching:fetchers>`{.interpreted-text role="ref"} 来获取源文件。正是 `SRC_URI`{.interpreted-text role="term"} 变量触发了 fetcher。任务 `ref-tasks-patch`{.interpreted-text role="ref"} 在源文件被获取后使用变量来应用补丁。OpenEmbedded 构建系统使用 `FILESOVERRIDES`{.interpreted-text role="term"} 来扫描 `SRC_URI`{.interpreted-text role="term"} 中的本地文件所在的目录位置。
...
> `SRC_URI`{.interpreted-text role="term"}变量在您的配方中必须定义每个源文件的唯一位置。在 `SRC_URI`{.interpreted-text role="term"}中不要硬编码版本号。与其硬编码这些值，请使用 `${``PV`{.interpreted-text role="term"}`}`，这会导致获取过程使用配方文件名中指定的版本。以这种方式指定版本意味着将配方升级到未来版本只需将配方重命名为与新版本匹配即可。
...
> 这里是来自 `meta/recipes-devtools/strace/strace_5.5.bb` 食谱的简单示例，源文件来自单个 tarball。注意使用 `PV`{.interpreted-text role="term"}变量：
...
> 另一种指定源的方式是来自 SCM。对于 Git 存储库，您必须指定 SRCREV，并且应该指定 PV 以包括 SRCPV 中的修订。以下是来自食谱 meta / recipes-core / musl / gcompat_git.bb 的示例：
...
> 如果您尝试构建配方时没有指定 `SRC_URI`{.interpreted-text role="term"}校验和，或者提供了不正确的校验和，则构建将为每个缺失或不正确的校验和生成错误。作为错误消息的一部分，构建系统提供与获取的文件对应的校验和字符串。一旦您获得正确的校验和，您就可以将它们复制粘贴到您的配方中，然后再次运行构建以继续。
...
> 当您使用 `file://` URI 协议指定本地文件时，构建系统会从本机获取文件。路径是相对于 `FILESPATH`{.interpreted-text role="term"}变量的，并且按特定顺序搜索特定的目录：`${``BP`{.interpreted-text role="term"}`}`，`${``BPN`{.interpreted-text role="term"}`}` 和 `files`。这些目录被假定为菜谱或附加文件所在目录的子目录。有关指定这些类型文件的另一个示例，请参见“[构建单个.c 文件包](#building-a-single-.c-file-package)”部分。
...
> 在构建过程中，`ref-tasks-unpack`{.interpreted-text role="ref"}任务会使用 `${``S`{.interpreted-text role="term"}`}` 指向解压缩的源代码。
...
> 如果你从上游源的归档 tarball 中提取源文件，并且 tarball 的内部结构符合顶级子目录命名为 `${BPN}-${PV}` 的共同惯例，那么你不需要设置 S。但是，如果 SRC_URI 指定从不使用此惯例的归档文件或像 Git 或 Subversion 这样的 SCM 中获取源文件，则你的配方需要定义 S。
...
> 如果使用 BitBake 成功处理您的配方，解压源文件后，您需要确保由 `${S}` 指向的目录与源文件的结构匹配。
...
> 有时需要在获取代码后补丁。任何在 `SRC_URI`{.interpreted-text role="term"}中提到的文件名以 `.patch` 或 `.diff` 结尾，或这些后缀的压缩版本（例如 `diff.gz`）都被视为补丁。`ref-tasks-patch`{.interpreted-text role="ref"}任务会自动应用这些补丁。
...
> 系统构建应能够使用“-p1”选项应用补丁（即在路径中将一个目录级别剥离）。 如果您的补丁需要剥离更多的目录级别，请在补丁的 `SRC_URI`{.interpreted-text role="term"}条目中使用“striplevel”选项指定级别数。 或者，如果您的补丁需要在补丁文件中未指定的特定子目录中应用，请在条目中使用“patchdir”选项。
...
> `LICENSE`{.interpreted-text role="term"}：这个变量指定软件的许可证。如果您不知道您正在构建的软件的发布许可证，您应该转到源代码并查找该信息。包含此信息的典型文件包括 `COPYING`、`LICENSE`{.interpreted-text role="term"}和 `README` 文件。您还可以在源文件顶部找到信息。例如，给定一款根据 GNU 通用公共许可证版本 2 发布的软件，您将如下设置 `LICENSE`{.interpreted-text role="term"}：
...
> 您在 `LICENSE`{.interpreted-text role="term"}中指定的许可证可以有任何名称，只要您不使用空格，因为空格用作许可证名称之间的分隔符。对于标准许可证，请使用 `meta/files/common-licenses/` 中的文件名或 `meta/conf/licenses.conf` 中定义的 `SPDXLICENSEMAP`{.interpreted-text role="term"}标志名称。
...
> 你需要为软件指定所有适用的许可文件。在配置步骤结束时，构建过程将比较文件的校验和，以确保文本没有更改。任何差异都会导致错误，消息中包含当前的校验和。有关如何设置 `LIC_FILES_CHKSUM`{.interpreted-text role="term"}变量的更多解释和示例，请参阅“`dev-manual/licenses:tracking license changes`{.interpreted-text role="ref"}”部分。
...
> 另一个考虑是，配置脚本可能会自动检查可选依赖项并在发现这些依赖项时启用相应的功能。如果您希望制作一个更通用的食谱（例如，在层中发布食谱供其他人使用），而不是强制禁用功能，您可以使用 `PACKAGECONFIG` 变量，以便其他食谱用户可以轻松地启用和禁用功能及其相应的依赖项。
...
> 相似于构建时依赖，您可以通过变量 `RDEPENDS`{.interpreted-text role="term"}来指定运行时依赖。所有特定于包的变量都需要在末尾添加覆盖包的名称。由于配方的主包与配方同名，而配方的名称可以通过 `${``PN`{.interpreted-text role="term"}`}` 变量找到，因此您可以通过设置 `RDEPENDS:${PN}` 来指定主包的依赖关系。如果包的名称为 `${PN}-tools`，则您可以设置 `RDEPENDS:${PN}-tools`，等等。
...
> 主要的构建时间配置是关于检查构建时间依赖性，并可能根据此结果启用可选功能。您需要在配方的 `DEPENDS`{.interpreted-text role="term"}值中指定构建的软件的任何构建时间依赖性，以其他满足这些依赖性的配方为基础。您通常可以在软件的文档中找到构建时间或运行时依赖性。
...
> 当使用自动工具时，您的配方需要继承 `ref-classes-autotools`{.interpreted-text role="ref"}类，而且不需要包含 `ref-tasks-configure`{.interpreted-text role="ref"}任务。但是，您仍然可能需要做一些调整。例如，您可以设置 `EXTRA_OECONF`{.interpreted-text role="term"}或 `PACKAGECONFIG_CONFARGS`{.interpreted-text role="term"}以传递任何特定于配方的需要的配置选项。
...
> 当你使用 CMake 时，你的配方需要继承 `ref-classes-cmake`{.interpreted-text role="ref"}类，而且不需要包含 `ref-tasks-configure`{.interpreted-text role="ref"}任务。你可以通过设置 `EXTRA_OECMAKE`{.interpreted-text role="term"}来进行一些调整，以传递特定于配方的任何所需的配置选项。
...
> 如果您需要安装由构建的应用程序提供的一个或多个自定义 CMake 工具链文件，请在 `ref-tasks-install` 期间将文件安装到 `${D}${datadir}/cmake/Modules`。
> :::
...
> 如果您的源文件没有 `configure.ac` 或 `CMakeLists.txt` 文件，那么您的软件是使用除 Autotools 或 CMake 以外的其他方法构建的。 如果是这种情况，您通常需要在配方中提供一个 `ref-tasks-configure`{.interpreted-text role="ref"}任务，当然，如果没有配置，也不需要。
...
> 一旦配置成功，最好查看 `log.do_configure` 文件以确保已启用适当的选项，并且不需要在 `DEPENDS` 中添加其他构建时依赖项。例如，如果配置脚本报告发现了 `DEPENDS` 中没有提及的东西，或者没有找到某些必需的可选功能，则需要将它们添加到 `DEPENDS` 中。查看日志也可能会发现您不想要的正在检查、启用或两者都有的项目，或者没有发现 `DEPENDS` 中的项目，在这种情况下，您需要按照需要向配置脚本传递额外的选项。要查阅与构建的软件相关的配置选项参考信息，可以查看 `${S}` 中的 `./configure --help` 命令的输出，或参考软件的上游文档。
...
> 在构建过程中，`ref-tasks-compile`{.interpreted-text role="ref"} 任务在源码被获取、解压缩和配置之后发生。如果配方成功通过 `ref-tasks-compile`{.interpreted-text role="ref"}，则无需做任何事情。
...
> 偶尔，需要应用补丁到源代码以确保使用正确的路径。如果需要指定路径来查找从其他配方阶段到 sysroot 的文件，请使用 OpenEmbedded 构建系统提供的变量（例如 `STAGING_BINDIR`{.interpreted-text role="term"}, `STAGING_INCDIR`{.interpreted-text role="term"}, `STAGING_DATADIR`{.interpreted-text role="term"}等）。
...
> 在 `ref-tasks-install`{.interpreted-text role="ref"}期间，任务会将编译文件及其层级一起复制到与目标设备上的位置相匹配的位置。安装过程会将来自 `${``S`{.interpreted-text role="term"}`}`、`${``B`{.interpreted-text role="term"}`}` 和 `${``WORKDIR`{.interpreted-text role="term"}`}` 目录的文件复制到 `${``D`{.interpreted-text role="term"}`}` 目录，以创建与目标系统上应有的结构相同的结构。
...
> 如果你的食谱正在构建的软件使用 Autotools 或 CMake，OpenEmbedded 构建系统了解如何安装软件。因此，您不必在食谱中拥有 `ref-tasks-install`{.interpreted-text role="ref"}任务。您只需确保构建的安装部分没有问题即可。但是，如果您希望安装 `make install` 未安装的其他文件，则应使用“手册”中介绍的 `do_install:append` 函数使用安装命令来进行此操作。
...
> 你需要在你的配方中定义一个 `ref-tasks-install`{.interpreted-text role="ref"} 函数。该函数应调用 `oe_runmake install`，并可能需要传递目标目录。如何传递该路径取决于正在运行的 `Makefile` 的写法（例如 `DESTDIR=${D}`、`PREFIX=${D}`、`INSTALLROOT=${D}` 等）。
...
> 为了查看使用“make install”的示例食谱，请参阅“dev-manual / new-recipe：构建基于 makefile 的包”部分。
...
> 你需要在你的配方中定义一个 `ref-tasks-install`{.interpreted-text role="ref"}函数。该函数必须首先使用 `install -d` 在 `${``D`{.interpreted-text role="term"}`}` 下创建目录。一旦目录存在，你的函数可以使用 `install` 手动将构建的软件安装到目录中。
...
> 对于不使用 Autotools 或 CMake 的场景，您需要跟踪安装，诊断和修复任何问题，直到一切正常安装。您需要查看默认位置 `${D}`，即 `${WORKDIR}/image`，以确保您的文件已正确安装。
...
> 在安装过程中，您可能需要修改一些已安装文件以适应目标布局。例如，您可能需要用构建系统提供的变量值替换 initscript 中的硬编码路径，例如将'/usr/bin/'替换为'${bindir}'。如果您在 `ref-tasks-install`{.interpreted-text role="ref"}中执行此类修改，请确保在复制后而不是复制前修改目标文件。修改后复制可确保构建系统在需要时可以重新执行 `ref-tasks-install`{.interpreted-text role="ref"}。
...
> `- ` oe_runmake install `，可以直接运行，也可以通过` ref-classes-autotools `{.interpreted-text role="ref"}和` ref-classes-cmake `{.interpreted-text role="ref"}类间接运行，可以并行运行` make install `。有时，Makefile中可能缺少目标之间的依赖关系，可能会导致竞争条件。如果您在` ref-tasks-install`{.interpreted-text role="ref"}期间遇到间歇性失败，您可以通过在配方中添加以下内容来禁用并行 Makefile 安装来解决这些问题：
...
> 如果你需要安装一个或多个由您正在构建的应用程序提供的自定义 CMake 工具链文件，请在 `ref-tasks-install` 期间将文件安装到 `${D}${datadir}/cmake/Modules`。
> :::
...
> 如果您正在添加服务，而服务初始化脚本或服务文件本身未安装，则必须使用“do_install：append”功能在您的食谱中提供安装。如果您的食谱已经有一个“ref-tasks-install”{.interpreted-text role="ref"}功能，请在其末尾更新该功能，而不是添加额外的“do_install：append”功能。
...
> 你需要在你的食谱中设置 `INITSCRIPT_PACKAGES`{.interpreted-text role="term"}、`INITSCRIPT_NAME`{.interpreted-text role="term"}和 `INITSCRIPT_PARAMS`{.interpreted-text role="term"}变量。
...
> 要使用 systemd 启用一项服务，您的食谱需要继承 `ref-classes-systemd`{.interpreted-text role="ref"} 类。有关更多信息，请参见位于您的源目录{.interpreted-text role="term"} 中的 `systemd.bbclass` 文件。
...
> - *拆分文件*：`ref-tasks-package`{.interpreted-text role="ref"} 任务将配方生成的文件拆分为逻辑组件。即使是生成单个二进制文件的软件也可能包含调试符号、文档和其他逻辑组件，应该进行拆分。`ref-tasks-package`{.interpreted-text role="ref"} 任务确保文件被正确拆分和打包。
...
> -*运行质量检查*: `ref-classes-insane`{.interpreted-text role="ref"}类将一个步骤添加到包生成过程中，以便 OpenEmbedded 构建系统生成输出质量保证检查。此步骤执行一系列检查，以确保构建的输出免受运行时出现的常见问题的影响。有关这些检查的信息，请参阅 `ref-classes-insane`{.interpreted-text role="ref"}类和 Yocto 项目参考手册中的“`ref-manual/qa-checks:qa错误和警告消息`{.interpreted-text role="ref"}”章节。
...
> *检查您的软件包*：在构建您的软件之后，您需要确保您的软件包是正确的。检查 `${``WORKDIR`{.interpreted-text role="term"}`}/packages-split` 目录，确保文件在您期望的位置。如果发现问题，可以根据需要设置 `PACKAGES`{.interpreted-text role="term"}、`FILES`{.interpreted-text role="term"}、`do_install(:append)` 等。
...
> 默认情况下，软件包适用于与目标机器具有相同架构的任何机器。当配方产生专用于某台机器的软件包（例如将 `MACHINE`{.interpreted-text role="term"}值传递给配置脚本或仅为特定机器应用补丁）时，您应通过添加以下内容来将其标记为：
...
> 另一方面，如果食谱不包含任何特定于目标机器或架构的内容（例如，仅打包脚本文件或配置文件的食谱），您应该使用 `ref-classes-allarch`{.interpreted-text role="ref"}类来为您做到这一点，方法是将其添加到您的食谱中：
...
> 食谱通常需要使用构建主机上其他食谱提供的文件。例如，链接到公共库的应用程序需要访问库本身及其关联的头文件。实现此访问的方法是通过填充 sysroot 文件。每个食谱在其工作目录中有两个 sysroot，一个用于目标文件（“recipe-sysroot”），另一个用于构建主机本地文件（“recipe-sysroot-native”）。
...
> 食谱不应该直接填充 sysroot（即在 sysroot 中写入文件）。相反，文件应该在 `ref-tasks-install` 任务期间安装到标准位置，该任务位于 `${``D`}` 目录中。这个限制的原因是几乎所有填充 sysroot 的文件都在清单中进行了目录，以确保在食谱被修改或删除时可以将文件删除。因此，sysroot 可以保持不受陈旧文件的影响。
...
> 只有[/sysroot]{.title-ref}被用于生成不包含在目标文件系统中的工件的配方，从而允许它们在不需要使用 `DEPLOY_DIR`{.interpreted-text role="term"}的情况下共享这些工件。
> :::
...
> 对于 `ref-tasks-populate_sysroot`{.interpreted-text role="ref"}任务及其相关功能的更完整描述，请参见 `staging <ref-classes-staging>`{.interpreted-text role="ref"}类。
...
> 安装后脚本会在安装包到目标上或在创建镜像时（如果包已包含在镜像中）立即运行。要将后安装脚本添加到包中，请在配方文件（`.bb`）中添加一个 `pkg_postinst:`[PACKAGENAME]{.title-ref}`()` 函数，并将[PACKAGENAME]{.title-ref}替换为您要附加到 `postinst` 脚本的包的名称。要将后安装脚本应用于配方的主包（通常是所需的），请在 PACKAGENAME 的位置指定 `${``PN`{.interpreted-text role="term"}`}`。
...
> 有时，必须将安装后脚本的执行推迟到第一次启动时才执行。例如，脚本可能需要在设备本身上执行。要将脚本执行推迟到启动时，必须明确标记将安装后任务延迟到目标。您可以使用 `pkg_postinst_ontarget()` 或从 `pkg_postinst()` 调用 `postinst_intercept delay_to_first_boot`。`ref-tasks-rootfs`{.interpreted-text role="ref"}任务期间，任何 `pkg_postinst()` 脚本的失败（包括退出 1）都会引发错误。
...
> 如果您的食谱中使用了 `pkg_postinst` 函数，并且需要在根文件系统构建期间使用非标准本机工具，则需要在食谱中使用 `PACKAGE_WRITE_DEPS` 变量来列出这些工具。如果您不使用此变量，则可能会缺少工具，并且在第一次启动时延迟执行后安装脚本。延迟脚本到第一次启动是不可取的，对于只读根文件系统是不可能的。
...
> 有相等的支持，可以通过 `pkg_preinst`、`pkg_prerm` 和 `pkg_postrm` 分别预安装、预卸载和后卸载脚本。这些脚本的工作方式与 `pkg_postinst` 完全相同，只是运行的时间不同。此外，由于运行的时间，它们不适用于像 `pkg_postinst` 一样在图像创建时运行。
> :::
...
> 最后一步完成你的食谱是确保你构建的软件运行正确。为了完成运行时测试，将构建的输出包添加到你的图像中，并在目标上进行测试。
...
> 要了解如何通过添加特定的软件包来定制图像的信息，请参阅“dev-manual/customizing-images：定制图像”部分。
...
> 应用程序使用 GNU `make` 构建时，需要在 `SRC_URI`{.interpreted-text role="term"}中列出源存档。您不需要添加 `ref-tasks-compile`{.interpreted-text role="ref"}步骤，因为 BitBake 默认启动 `make` 命令来编译应用程序。如果您需要额外的 `make` 选项，应将它们存储在 `EXTRA_OEMAKE`{.interpreted-text role="term"}或 `PACKAGECONFIG_CONFARGS`{.interpreted-text role="term"}变量中。BitBake 将这些选项传递到 GNU `make` 调用中。请注意，仍然需要 `ref-tasks-install`{.interpreted-text role="ref"}任务。否则，BitBake 默认运行空的 `ref-tasks-install`{.interpreted-text role="ref"}任务。
...
> 应用程序使用 Autotools 构建，例如 `autoconf` 和 `automake`，需要一个 `SRC_URI`{.interpreted-text role="term"}中列出的源存档，并继承 `ref-classes-autotools`{.interpreted-text role="ref"}类，该类包含构建基于 Autotool 的应用程序所需的所有步骤的定义。构建的结果会自动打包。如果应用程序使用 NLS 进行本地化，则会生成具有本地信息的软件包（每种语言一个软件包）。以下是一个示例：（`hello_2.3.bb`）：
...
> 这个:oe_[git:%60ipcalc](git:%60ipcalc) 食谱 </meta-openembedded/tree/meta-networking/recipes-support/ipcalc>` 是一个没有依赖的简单应用程序的例子。
...
> 应用程序有依赖性时很可能继承 `ref-classes-pkgconfig`{.interpreted-text role="ref"}类，因为 `pkg-config` 是 Meson 用来查找依赖项并将应用程序编译与它们相关联的默认方法。
...
> 你可以使用变量 `PACKAGES`{.interpreted-text role="term"} 和 `FILES`{.interpreted-text role="term"} 将应用程序分割成多个包。
...
> 以下是使用 `libxpm` 食谱的示例。默认情况下，此食谱生成一个包，其中包含库以及几个可执行文件。您可以修改食谱以将可执行文件拆分成单独的包：
...
> 在前面的例子中，我们希望将 `sxpm` 和 `cxpm` 二进制文件打包到不同的包中。由于 `bindir` 默认情况下会打包到主 `PN`{.interpreted-text role="term"}包中，我们在 `PACKAGES`{.interpreted-text role="term"}变量前面加上了额外的包名字，这样就可以将其添加到列表的开头。结果，额外的 `FILES:*` 变量就包含了定义哪些文件和目录被打包到哪个包中的信息。被前面的包包含的文件会被后面的包跳过。因此，主 `PN`{.interpreted-text role="term"}包不会包含上面列出的文件。
...
> 有时，您需要将预编译的二进制文件添加到映像中。例如，假设存在由公司的特定部门创建的专有代码的二进制文件。您的公司部分需要将这些二进制文件作为使用 OpenEmbedded 构建系统构建的映像的一部分使用。由于您只有二进制文件而没有源代码，因此无法使用期望获取 `SRC_URI`{.interpreted-text role="term"}中指定的源代码并进行编译的典型配方。
...
> 一种方法是将二进制文件打包，然后将它们作为图像的一部分安装。一般来说，打包二进制文件不是一个好主意，因为在其他方面，它会阻碍重现构建的能力，并可能导致将来与 ABI 的兼容性问题。但是，有时您别无选择。
...
> 最简单的解决方案是创建一个使用 `ref-classes-bin-package`{.interpreted-text role="ref"}类的配方，并确保您正在使用构建构件的默认位置。在大多数情况下，`ref-classes-bin-package`{.interpreted-text role="ref"}类处理“跳过”配置和编译步骤，并设置从适当区域抓取软件包的事情。特别是，这个类将 `noexec` 设置到 `ref-tasks-configure`{.interpreted-text role="ref"}和 `ref-tasks-compile`{.interpreted-text role="ref"}任务上，将 `FILES:${PN}` 设置为“/”，以便它从 `${S}` 中捡起所有文件，并设置一个 `ref-tasks-install`{.interpreted-text role="ref"}任务，它有效地将所有文件从 `${S}` 复制到 `${D}`。当提取到 `${S}` 中的文件已经按照在目标上应该放置的方式排列时，`ref-classes-bin-package`{.interpreted-text role="ref"}类工作良好。有关这些变量的更多信息，请参阅 Yocto 项目参考手册的变量词汇表中的 `FILES`{.interpreted-text role="term"}、`PN`{.interpreted-text role="term"}、`S`{.interpreted-text role="term"}和 `D`{.interpreted-text role="term"}变量。
...
> 创建一个食谱，其中 `ref-tasks-configure`{.interpreted-text role="ref"}和 `ref-tasks-compile`{.interpreted-text role="ref"}任务什么都不做：通常只需不在食谱中定义这些任务即可，因为默认实现除非在 ${``S `{.interpreted-text role="term"}`}中找到 Makefile，否则不会做任何事情。
...
> 如果 `${S}` 可能包含一个 Makefile，或者如果您继承了一个类，用自定义版本替换 `ref-tasks-configure`{.interpreted-text role="ref"}和 `ref-tasks-compile`{.interpreted-text role="ref"}，那么您可以使用 `[``noexec <bitbake-user-manual/bitbake-user-manual-metadata:variable flags>`{.interpreted-text role="ref"}`]` 标志将任务转换为无操作，如下所示：
...
> 与使用标记不同，使用 bitbake-user-manual / bitbake-user-manual-metadata：删除任务{.interpreted-text role="ref"}可以保留从 ref-tasks-fetch{.interpreted-text role="ref"}、ref-tasks-unpack{.interpreted-text role="ref"}和 ref-tasks-patch{.interpreted-text role="ref"}任务到 ref-tasks-install{.interpreted-text role="ref"}任务的依赖链。
...
> 写食谱时，最好遵守现有的样式指南。 :oe_wiki:[OpenEmbedded Styleguide \</Styleguide\>]{.title-ref} 维基页面提供了首选食谱样式的大致指南。
...
> 一般来说，现有食谱会有一定的偏差。不过，尽量保持一致的风格是个好主意。某些做法，比如在赋值运算符 `=` 两边省略空格，或者把食谱组件以不规则的方式排列，都被广泛认为是不好的风格。
...
> - *关键词：*BitBake 食谱只使用几个关键词。您可以使用关键词来包括常见函数（`inherit`），从其他文件加载食谱的部分（`include` 和 `require`）以及将变量导出到环境（`export`）。
...
> 下一个列表总结了食谱语法中最重要和最常用的部分。要了解更多关于语法的信息，您可以参考 BitBake 用户手册中的“bitbake：bitbake-user-manual / bitbake-user-manual-metadata”章节。
...
> *覆盖：*您可以使用覆盖来根据构建菜谱的方式有条件地设置值。例如，要将 `KBRANCH`{.interpreted-text role="term"}变量的值设置为“standard/base”，除了 qemuarm，其值应设置为“standard/arm-versatile-926ejs”，您可以执行以下操作：
...
> 覆盖也用于在其他情况下分离变量的替代值。例如，当设置特定于由配方生成的单个包的变量（如 `FILES`{.interpreted-text role="term"}和 `RDEPENDS`{.interpreted-text role="term"}）时，您应始终使用指定包名称的覆盖。
