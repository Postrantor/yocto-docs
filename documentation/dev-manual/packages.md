...
> 技术上，第三个组件“epoch”（即 `PE`{.interpreted-text role="term"}）也涉及其中，但本讨论大部分忽略了 `PE`{.interpreted-text role="term"}。
> :::
...
> `PV`{.interpreted-text role="term"}：食谱版本。`PV`{.interpreted-text role="term"}代表正在打包的软件的版本。不要把 `PV`{.interpreted-text role="term"}与二进制包版本混淆。
...
> 按照提到的，试图在元数据中维护修订号是容易出错、不准确的，并且给提交食谱的人带来问题。相反，PR 服务会自动生成递增的编号，特别是修订字段，这消除了人为因素。
...
> 项目 Yocto 使用变量按照递减的优先级来促进版本号的编号（即使用“PE”{.interpreted-text role="term"}、“PV”{.interpreted-text role="term"}和“PR”{.interpreted-text role="term"}分别表示历史、版本和修订）。这些值高度依赖于给定分发和软件包源的政策和程序。
...
> 默认情况下，PR 服务未启用或未运行。因此，生成的软件包只是“自我一致”的。构建系统添加和删除软件包，关于升级路径没有保证，但图像将与最新更改保持一致和正确。
...
> 如果提交的更改导致更改包输出，则需要作为该提交的一部分增加（或“提升”）`PR`{.interpreted-text role="term"}变量的值。对于新配方，您应添加 `PR`{.interpreted-text role="term"}变量，并将其初始值设置为“r0”，这是默认值。尽管默认值为“r0”，但在将来更改配方时，添加它以防止忘记提升变量的做法是有益的。
...
> 为确保模块打包实际得以完成，您可以在食谱中使用 `populate_packages` Python 函数内的 `do_split_packages` 函数。`do_split_packages` 函数会搜索指定路径下的文件或目录的模式，并为其中每一个找到的项目创建一个包，通过附加 `PACKAGES`{.interpreted-text role="term"}变量并设置 `FILES:packagename`、`RDEPENDS:packagename`、`DESCRIPTION:packagename` 等适当的值。以下是 `lighttpd` 食谱中的一个示例：
...
> 给 `extra_depends` 一个空字符串，这将禁用对主 `lighttpd` 软件包的默认依赖关系。因此，如果在 `${libdir}` 中找到名为 `mod_alias.so` 的文件，则会为其创建一个名为 `lighttpd-module-alias` 的软件包，并将 `DESCRIPTION` 设置为“Lighttpd 模块别名”。
...
> 在构建过程中，BitBake 总是将配方转换为一个或多个软件包。例如，BitBake 会处理 `bash` 配方，生成许多软件包（例如 `bash`、`bash-bashbug`、`bash-completion`、`bash-completion-dbg`、`bash-completion-dev`、`bash-completion-extra`、`bash-dbg` 等等）。但是，不是所有生成的软件包都会包含在图像中。
...
> 如果您希望图像以包含当前构建中的包以及在目标上具有运行时包管理所需的相关工具的基本软件包数据库开始，您可以在 `IMAGE_FEATURES` 变量中包括“package-management”。 在此配置变量中包括“package-management”可确保，当为您的目标组装图像时，图像将包括当前已知的软件包数据库以及用于在目标上执行运行时包管理所需的特定于目标的工具。 但是，这不是必需的。 您可以在没有任何数据库的情况下开始您的图像，但只包括所需的目标软件包工具。 例如，如果您使用 IPK 软件包格式，可以将“opkg”包括在您的 `IMAGE_INSTALL` 变量中。 然后，可以在图像启动后稍后初始化目标的软件包数据库。
...
> 你可以使用 `PACKAGE_FEED_ARCHS`{.interpreted-text role="term"}、`PACKAGE_FEED_BASE_PATHS`{.interpreted-text role="term"}和 `PACKAGE_FEED_URIS`{.interpreted-text role="term"}变量来预先配置目标图像以使用软件包源。如果您没有定义这些变量，则需要按照后面部分中描述的步骤手动配置目标。为了生成正确配置的图像，您应该在构建图像之前设置这些变量。
...
> 当构建完成后，您的软件包将位于 `${TMPDIR}/deploy/packageformat` 目录中。例如，如果 `${TMPDIR}` 是 `tmp`，而您选择的软件包类型是 RPM，那么您的 RPM 软件包可在 `tmp/deploy/rpm` 中找到。
...
> 从构建目录（即 `PACKAGE_CLASSES` 设置）中，根据您的打包选择构建的图像，只需启动服务器即可。 以下示例假定 `Build Directory` 为 `poky/build`，`PACKAGE_CLASSES` 设置为“`ref-classes-package_rpm`”：
...
> DNF（Dandified Packaging）是一种运行时包管理 RPM 包的工具。为了使用 DNF 进行运行时包管理，您必须在目标机器上执行初始设置，以处理在构建图像时没有使用 PACKAGE_FEED_*变量的情况。这意味着，如果您构建了图像，没有使用这些变量作为构建的一部分，并且您的图像现在正在目标上运行，则如果您想要使用运行时包管理，则需要执行本节中的步骤。
...
> 对于 `PACKAGE_FEED_*` 变量的信息，请参阅 Yocto 项目参考手册变量词汇表中的 `PACKAGE_FEED_ARCHS`{.interpreted-text role="term"}、`PACKAGE_FEED_BASE_PATHS`{.interpreted-text role="term"}和 `PACKAGE_FEED_URIS`{.interpreted-text role="term"}。
> :::
...
> 应用程序 `opkg` 对 IPK 软件包进行运行时包管理。如果变量 `PACKAGE_FEED_ARCHS`{.interpreted-text role="term"}, `PACKAGE_FEED_BASE_PATHS`{.interpreted-text role="term"}和 `PACKAGE_FEED_URIS`{.interpreted-text role="term"}尚未设置，或者目标映像在设置变量之前构建，则必须在目标机器上进行 `opkg` 的初始设置。
...
> `apt` 应用程序执行 DEB 包的运行时包管理。此应用程序使用源列表文件来查找可用的软件包数据库。如果未设置 `PACKAGE_FEED_ARCHS`{.interpreted-text role="term"}、`PACKAGE_FEED_BASE_PATHS`{.interpreted-text role="term"}和 `PACKAGE_FEED_URIS`{.interpreted-text role="term"}变量，或者在设置变量之前构建了目标映像，则必须在目标机器上执行 `apt` 的初始设置。
...
> 一旦构建完成，ptest 文件就会安装到图像中的 `/usr/lib/package/ptest` 目录中，其中 `package` 是包的名称。
...
> `ptest-runner` 包安装了一个 shell 脚本，可以循环遍历所有安装的 ptest 测试套件，并依次运行它们。因此，您可能需要将此包添加到您的图像中。
...
> 要使食谱在目标硬件上运行已安装的 ptests，您需要准备构建您要测试的软件包的食谱。以下是您为每个食谱必须做的事情：
...
> *创建 run-ptest：*此脚本会启动您的测试。使用 `SRC_URI`{.interpreted-text role="term"}将脚本定位到您要引用它的位置。以下是一个启动 `dbus` 测试的示例：
...
> 安装测试套件：`ref-classes-ptest`{.interpreted-text role="ref"}类自动将文件 `run-ptest` 复制到目标，然后运行 make `install-ptest` 来运行测试。如果这还不够，您需要创建一个 `do_install_ptest` 函数，并确保它在“make install-ptest”完成后调用。
...
> 需要使用 NPM 主机工具，需要安装 OpenEmbedded 环境中的原生 nodejs-npm 包。您需要通过克隆 oe_[git：`meta-openembedded`](git%EF%BC%9A%60meta-openembedded%60) \</meta-openembedded\>仓库来获取该包。一定要将本地副本的路径添加到您的[bblayers.conf]{.title-ref}文件中。
...
> 命令 `devtool add` 运行 `recipetool create`，并使用相同的获取 URI 下载每个依赖项并尽可能捕获许可证详细信息。结果是生成的食谱。
...
> 这个命令生成的配方与前一节中创建的配方非常相似。但是，`SRC_URI`{.interpreted-text role="term"} 看起来像下面这样：
...
> 在这个例子中，主模块是从 Git 存储库中获取的，而依赖项是从 NPM 注册表中获取的。除了这些差异之外，两种方法之间的食谱基本上是相同的。您可以按照使用注册表模块方法的前面一节中所述的方式构建和部署包。
